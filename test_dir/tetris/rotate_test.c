#include "../../brick_game/tetris/include/moving/rotate.h"
...
START_TEST(tmino_spin_1) {
  ...
  tmino_spin(tg_info, &pos, &tmino_fell);
...
Suite *rotate_suite(void) {
  Suite *s;
  TCase *tc_rotate;
  s = suite_create("rotate_test_suite");
  tc_rotate = tcase_create("rotate_test_suite");

  tcase_add_test(tc_rotate, tmino_spin_1);
  tcase_add_test(tc_rotate, tmino_spin_2);
  tcase_add_test(tc_rotate, tmino_spin_3);
