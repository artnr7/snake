...
START_TEST(g_info_init_1) {
  ...
  fld_malloc_and_fill(&(test_tg_info.field), FIELD_H, FIELD_W);
  fld_malloc_and_fill(&(test_tg_info.next), NEXT_TMINO_H, NEXT_TMINO_W);
  ...
  g_info_and_nxt_frm_free(&test_tg_info);
}
END_TEST
