#include "../../brick_game/tetris/include/collision.h"
...
START_TEST(is_blk_insd_1) {
  for (int k = -3; k < FIELD_H + 3; k++) {
    for (int l = -3; l < FIELD_W + 3; l++) {
      if ((k >= 0 && k < FIELD_H) && (l >= 0 && l < FIELD_W)) {
...
Suite *collision_suite(void) {
  Suite *s;
  TCase *tc_collision;
  s = suite_create("collision_test_suite");
  tc_collision = tcase_create("collision_test_suite");
