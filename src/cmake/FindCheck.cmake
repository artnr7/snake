...
# ::
#
# CHECK_FOUND          - True if Check found. CHECK_INCLUDE_DIRS   - Where to
# find check.h. CHECK_LIBRARIES      - List of libraries when using Check.
...
elseif(CHECK_INCLUDE_DIR AND EXISTS "${CHECK_INCLUDE_DIR}/check.h")
  ...
  foreach(v ${check_version_list})
    ...
    file(STRINGS "${CHECK_INCLUDE_DIR}/check.h" check_version_${v}
         REGEX "${regex_check_version}")
    string(REGEX REPLACE "${regex_check_version}" "\\1" check_version_${v}
                         "${check_version_${v}}")
    unset(regex_check_version)
  endforeach()
  set(CHECK_VERSION_STRING
      "${check_version_MAJOR}.${check_version_MINOR}.${check_version_MICRO}")
  foreach(v check_version_list)
