#include "../brick_game/tetris/tetris_include/tetris_speed_and_levels.h"
...
START_TEST(lvl_up_1) {
  ...
  ck_assert_int_eq(tg_info->speed, 500);
...
START_TEST(lvl_up_2) {
  ...
  ck_assert_int_eq(tg_info->speed, 100);
...
START_TEST(lvl_up_3) {
  ...
  ck_assert_int_eq(tg_info->speed, 50);
...
START_TEST(high_scr_get_1) {
  ...
  FILE *fp_1 = fopen("brick_game/tetris/high_score.txt", "w");
  if (fp) {
...
Suite *tetris_speed_and_levels_suite(void) {
  ...
  s = suite_create("tetris_speed_and_levels_test_suite");
  tc_speed_and_levels = tcase_create("tetris_speed_and_levels_test_suite");
