...
void shall_create_tmino(GameInfo_t *g_info, bool *tmino_fell, int *tmino_pos) {
   ...
   * падения меняется на ложный
   ▼ */
  if (can_create_tmino_fn(g_info, *tmino) && *tmino_fell == true) {
    fld_tmino_create(g_info, tmino, tmino_pos);
    *tmino_fell = false;
  }
}
  ...
  *tmino_pos = 0;  // ◄ инициализация позиции нового тетромино, всегда 0
  ...
  for (int i = 0; i < NEXT_TMINO_H; i++) {
    for (int j = 0; j < NEXT_TMINO_W; j++) {
      g_info->next[i][j] = Nothing;
    }
  }
  tmino_parser(g_info->next, *tmino, 0, 0, 0);
}
...
bool can_create_tmino_fn(GameInfo_t *g_info, const Tetromino_t tmino) {
  ...
  if (!tmino_parser(g_info->field, tmino, TMINO_START_Y, TMINO_START_X, 1)) {
    go_end(g_info);
