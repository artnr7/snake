#include "../../tetris_include/tetris_moving/tetris_rotate.h"
 ...
 * │....│
 * └────┘ */
void action_fn(GameInfo_t *tg_info, int *pos, bool *tmino_fell) {
  ...
  can_tmino_rot_fn(*tg_info, fst_blk_i, fst_blk_j, rot_mtx_h, rot_mtx_w,
                   &try_x_mv, &can_tmino_rot);

  /* ▼ Если тетромино рядом с нижней границей тетромино не может крутиться */
  can_tmino_rot = can_tmino_rot ? !bot_coll(*tg_info) : can_tmino_rot;
...
void fst_mvg_blk_find(GameInfo_t tg_info, int *fst_blk_i, int *fst_blk_j,
                      bool *can_tmino_rot) {
  ...
  for (int i = 0; i < FLD_H && braek == false; i++) {
    for (int j = 0; j < FLD_W && braek == false; j++) {
      if (mvg_px_dtrm(tg_info.field[i][j])) {
        if (tg_info.field[i][j] == MagentaOMvg) {
          *can_tmino_rot = false;
        }
        *fst_blk_i = i;
        *fst_blk_j = j;
...
void tmino_way_decis(GameInfo_t *tg_info, int **nxt_frm, int i, int j,
  ...
  if (can_tmino_rot) {
    /* Если тетромино не нужно двигаться по горизонтали чтобы вращаться,
     ...
     ▼ */
    if (!try_x_mv) {
      tmino_rot(tg_info, nxt_frm, i, j, rot_mtx_h, rot_mtx_w);
    ...
    /* Если тетромино нужно двигаться по горизонтали чтобы вращаться, значит
      ...
      tmino_shift_rot(tg_info, j, pos, tmino_fell, &can_tmino_rot);
    }
  }
}
...
void tmino_rot(GameInfo_t *tg_info, int **nxt_frm, int i, int j, int rot_mtx_h,
               int rot_mtx_w) {
  ...
  for (int k = i; k < rot_mtx_h; k++) {
    for (int l = j; l < rot_mtx_w; l++) {
      ...
      if (is_blk_insd(rot_mtx_i, rot_mtx_j) &&
          mvg_px_dtrm(tg_info->field[rot_mtx_i][rot_mtx_j])) {
        nxt_frm[k][l] = tg_info->field[rot_mtx_i][rot_mtx_j];
      }
    }
  }
}
 ...
 * повернётся
 */
void tmino_shift_rot(GameInfo_t *tg_info, int j, int *pos, bool *tmino_fell,
                     bool *can_tmino_rot) {
  int lr_way = 0;
