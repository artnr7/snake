#include "../tetris_include/tetris_collision.h"
  ...
  for (int k = i; k < rot_mtx_h && blk_qty <= TMINO_BLKS_QTY; k++) {
    for (int l = j; l < rot_mtx_w && blk_qty <= TMINO_BLKS_QTY; l++) {
      ...
      if (is_blk_insd(rot_mtx_i, rot_mtx_j) &&
        ...
        if (!is_blk_insd(k, l) ||
            (is_blk_insd(k, l) && stc_px_dtrm(tg_info.field[k][l]))) {
          *try_x_mv = true;
        }
  ...
  for (int i = 0; i < FLD_H && blk_qty <= TMINO_BLKS_QTY; i++) {
    for (int j = 0; j < FLD_W && blk_qty <= TMINO_BLKS_QTY; j++) {
      if (mvg_px_dtrm(tg_info.field[i][j])) {
        ...
        if (i == FLD_H - 1 ||
            (is_blk_insd(i + 1, j) && stc_px_dtrm(tg_info.field[i + 1][j]))) {
...
void lr_brd_tmino_coll(const GameInfo_t *tg_info,
  ...
  for (int i = 0; i < FLD_H && blk_qty <= TMINO_BLKS_QTY; i++) {
    for (int j = 0; j < FLD_W && blk_qty <= TMINO_BLKS_QTY; j++) {
      if (mvg_px_dtrm(tg_info->field[i][j])) {
        ...
        if (is_blk_insd(i, j - 1) && stc_px_dtrm(tg_info->field[i][j - 1])) {
